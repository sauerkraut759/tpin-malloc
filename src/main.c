...
{
	c->size = c->size | PREV_IN_USE;
}

__always_inline void *chunk_to_user(struct tpin_chunk *c)
{
...
{
	struct tpin_chunk *new = heap.top;
	new->size = sz;

	struct tpin_chunk *new_top = (struct tpin_chunk *)((uint8_t *)heap.top + sz);
	new_top->size = heap.top->size - sz;
